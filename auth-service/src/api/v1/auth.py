...
from schemas.auth import Credentials, TokenPair
...
async def login(
    ...
    tokens = await auth_service.login(credentials, user_agent)
    if not tokens:
...
async def refresh_tokens(
    ...
    try:
        await auth_jwt.jwt_refresh_token_required(auth_from='websocket',
                                                  token=refresh_token)
...
async def check_access(
    ...
    allow_roles: Annotated[list[Literal['admin', 'subscriber']],
                           Query(description='Кому доступно')] = None,
    auth_service: AuthService = Depends(get_auth_service),
) -> None:
