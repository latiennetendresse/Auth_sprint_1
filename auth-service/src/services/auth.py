...
class AuthService:
    ...
    async def login(
        self, credentials: Credentials, user_agent: Optional[str]
    ) -> Optional[TokenPair]:
    ...
    async def end_session(self, session_id: UUID, reason: str):
        result = await self.db.execute(
            ...
            .values(session_exp=datetime.utcnow()).returning(Session)
        )
        await self.db.commit()
    ...
    async def revoke_access_token(self, jti: UUID, reason: str):
        await self.redis.setex(
            str(jti), settings.authjwt_access_token_expires, reason
        )
    ...
    async def create_token_pair(
        ...
        refresh_token = await self.auth_jwt.create_refresh_token(
            subject=str(user.id),
            user_claims={'session_id': str(session_id)}
        )
        refresh_jwt = await self.auth_jwt.get_raw_jwt(refresh_token)
