...
async def test_delete(
    delete_role, create_role, list_roles, ivanov, admin_role, add_role,
    ivanov_login
):
...
async def test_unauthorized(admin_role, make_request):
    response = await make_request(
        hdrs.METH_DELETE, f"/api/v1/roles/{admin_role['id']}"
    )
...
async def test_not_found(
    delete_role, admin_role, ivanov, add_role, ivanov_login
):
    await add_role(ivanov['id'], admin_role['id'])
    access_token = (await ivanov_login())['access_token']
