...
async def test_patch(
    patch_role, create_role, list_roles, ivanov, admin_role, add_role,
    ivanov_login
):
...
async def test_unauthorized(admin_role, make_request):
    response = await make_request(
        hdrs.METH_PATCH, f"/api/v1/roles/{admin_role['id']}",
        json={'name': 'joker'}
    )
...
async def test_not_found(
    patch_role, admin_role, ivanov, add_role, ivanov_login
):
    await add_role(ivanov['id'], admin_role['id'])
    access_token = (await ivanov_login())['access_token']
