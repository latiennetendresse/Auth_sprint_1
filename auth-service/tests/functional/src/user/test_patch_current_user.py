...
@pytest.mark.parametrize(
        ...
        [
            (
                {'email': 'petrov@ya.ru'},
                {'email': 'petrov@ya.ru', 'name': 'Ivan Ivanov'}
            ),
            (
                {'name': 'Vasya Ivanov'},
                {'email': 'ivanov@ya.ru', 'name': 'Vasya Ivanov'}
            ),
        ]
)
async def test_update_info(
    ...
    response = await make_request(
        hdrs.METH_PATCH, '/api/v1/user', json=user_patch,
...
async def test_update_password(
    ivanov, ivanov_login, make_request
):
    access_token = (await ivanov_login())['access_token']

    response = await make_request(
        hdrs.METH_PATCH, '/api/v1/user', json={'password': 'newpass'},
        headers={'Authorization': f'Bearer {access_token}'}
    )
    assert response['status'] == HTTPStatus.OK
...
async def test_unauthorized(make_request):
    response = await make_request(
        hdrs.METH_PATCH, '/api/v1/user', json={'email': 'petrov@ya.ru'},
    )
    assert response['status'] == HTTPStatus.UNAUTHORIZED


async def test_user_deleted(
    ivanov, ivanov_login, db_delete, make_request
):
    access_token = (await ivanov_login())['access_token']
