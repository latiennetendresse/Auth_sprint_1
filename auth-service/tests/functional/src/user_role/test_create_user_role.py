...
async def test_create(
    create_user_role, ivanov, ivanov_login, petrov, petrov_login,
    admin_role, add_role, list_roles, refresh_tokens
):
...
async def test_unauthorized(make_request, ivanov, admin_role):
    response = await make_request(
        hdrs.METH_POST, f"/api/v1/users/{ivanov['id']}/roles",
...
async def test_forbidden(create_user_role, ivanov, ivanov_login, admin_role):
    ...
    response = await create_user_role(access_token,
                                      ivanov['id'],
                                      admin_role['id'])
    assert response['status'] == HTTPStatus.FORBIDDEN
...
async def test_user_not_found(
    ...
    response = await create_user_role(
        ivanov_token, str(uuid4()), admin_role['id']
    )
...
async def test_role_not_found(
    ...
    ivanov_token = (await ivanov_login())['access_token']
    response = await create_user_role(
        ivanov_token, petrov['id'], str(uuid4())
    )
    assert response['status'] == HTTPStatus.BAD_REQUEST
...
async def test_user_role_exists(
    ...
    ivanov_token = (await ivanov_login())['access_token']
    response = await create_user_role(
        ivanov_token, ivanov['id'], admin_role['id']
    )
    assert response['status'] == HTTPStatus.BAD_REQUEST
