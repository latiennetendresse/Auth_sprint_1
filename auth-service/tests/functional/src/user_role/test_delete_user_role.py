...
async def test_delete(
    delete_user_role, ivanov, admin_role, add_role, ivanov_login, list_roles,
    refresh_tokens
):
...
async def test_unauthorized(make_request, ivanov, admin_role):
    response = await make_request(
        hdrs.METH_DELETE, f"/api/v1/users/{ivanov['id']}/roles",
...
async def test_forbidden(delete_user_role, ivanov, ivanov_login, admin_role):
    ...
    response = await delete_user_role(access_token,
                                      ivanov['id'],
                                      admin_role['id'])
    assert response['status'] == HTTPStatus.FORBIDDEN
...
async def test_not_found(
    delete_user_role, admin_role, ivanov, add_role, ivanov_login
):
    await add_role(ivanov['id'], admin_role['id'])
    access_token = (await ivanov_login())['access_token']
